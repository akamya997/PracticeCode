...
int main()
{
    ...
    while(T--)
    {
        ...
        for(int i=2;i<=c;i++)
        {
            ...
            if(x<0)
                rk[i]=-x;
            else rem.emplace_back(x,i),tdis[i]=x;
        }
        ...
        for(int i=0;i<d;i++)
        {
            ...
            if(tdis[u]==tdis[v]) ans.push_back(1e6);
            else ans.push_back(abs(tdis[u]-tdis[v]));
        }
        pkas();
        print(ans);
    }
