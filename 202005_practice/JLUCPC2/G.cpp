...
int main()
{
    ...
    for(int i=0;i<n;i++)
    {
        for(int j=i+1;j<n;j++)
        {
            ...
            ll dy = p2.second - p1.second, dx = p2.first - p1.first;
            ll b0 = p1.second * p2.first - p1.first * p2.second;
            ll g1 = __gcd(dx, dy), g2 = __gcd(b0, __gcd(dx, dy));
            auto cur=make_pair(make_pair(dx / g2, dy / g2), b0 / g2);
            if(mp2.count(cur)) ok=1;
            mp2[cur]=1;
        }
    }
    if(ok) cout<<"fzj\n";
