...
void Union(int x,int y,int id)
{
    ...
    if(x!=y)
    {
        ...
        fa[y]=x;
        //dbg(xx,yy,id);
        G[xx].push_back(Edge{yy,id});
        G[yy].push_back(Edge{xx,id});
    }
}
bool vis[maxn];
vc<pi> p;
...
{
    vis[u]=1;
    sz[u]=p[u].second^(u?p[u-1].second:0);
    for(auto &e:G[u])
    {
        int v=e.to;
        if(!vis[v])
...
int main()
{
    ...
    init();
    for(int i=0;i<n;i++)
        sz[i]=p[i].second^(i?p[i-1].second:0);
    for(int i=0;i<m;i++)
    {
        int l,r;
        cin>>l>>r;
    ...
    for(int i=0;i<=n;i++)
    {
        if(Find(i)==i)
        {
            if(Find(n)==i)
            {
                if(sz[i]&1) p[n].second=p[n-1].second^1;
                continue;
            }
            if(sz[i]&1) ok=0;
        }
