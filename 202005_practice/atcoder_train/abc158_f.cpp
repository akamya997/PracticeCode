...
int mx[maxn<<2];
void modify(int p,int v,int L=0,int R=n-1,int o=1)
{
    if(L==R)
    {
        mx[o]=v;
...
int query(int l,int r,int L=0,int R=n-1,int o=1)
{
...
int main()
{
    ...
    for(int i=0;i<n;i++) a[i].first=getid(a[i].first);
    sort(a.begin(),a.end());
    for(int i=n-1;i>=0;i--)
    {
        int tar=all[a[i].first]+a[i].second;
        int id=getid(tar);
        id--;
        if(id<=a[i].first) R[i]=a[i].first;
        else R[i]=max(a[i].first,query(a[i].first+1,id));
        modify(i,R[i]);
        //dbg(i,R[i]);
    }
    vi dp(n+1);
