...
int main()
{
    ...
    {
        ll cur=(ll)i*x;
        ll up=cur+(ll)(n-1+n-i)*i/2*d;
        ll down=cur+(ll)i*(i-1)/2*d;
        //dbg(i,up,down);
        seg[i]={(int)(cur%d),down,up};
    }
    sort(seg.begin(),seg.end());
    ll ans=0;
    ans=((seg[0].r-seg[0].l)/d+1);
    ll prer=seg[0].r;
    for(int i=1;i<=n;i++)
    {
        if(seg[i].rem==seg[i-1].rem)
        {
            if(seg[i].l<=prer)
                ans+=max(0LL,(seg[i].r-prer)/d);
            else ans+=(seg[i].r-seg[i].l)/d+1;
            prer=max(prer,seg[i].r);
        }
        else{
            ans+=(seg[i].r-seg[i].l)/d+1;
            prer=seg[i].r;
        }
    }
    print(ans);
