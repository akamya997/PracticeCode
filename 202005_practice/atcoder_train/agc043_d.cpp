...
int main()
{
    int n,m;
    cin>>n>>m;
    for(int i=0;i<maxn;i++) inv[i]=quick(i,m-2,m);
    n*=3;
    const int var=maxn;
    dp[0][var]=1;
    for(int i=1;i<=n;i++)
    {
        for(int k=1;k<=3;k++)
        {
            ...
            for(int j=0;j<2*maxn;j++)
            {
                ...
                int delta=k==1?1:(k==2?-1:0);
                dp[i][j+delta]=(dp[i][j+delta]+dp[i-k][j]*inv[i])%m;
                //dbg(i,j+delta,dp[i][j+delta],dp[i-k][j]);
            }
        }
    }
    ll fac=1;
    for(int i=1;i<=n;i++) fac=fac*i%m;
    ll ans=0;
    for(int i=var;i<maxn*2;i++)
        ans=(ans+dp[n][i]*fac%m)%m;
    print(ans);
