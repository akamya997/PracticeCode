...
int main()
{
    ...
    cin>>n>>k;
    vc<pi> sh(n);
    for(int i=0;i<n;i++) cin>>sh[i].first>>sh[i].second;
    vc<ll> dp(k,-1e9);
    ll tot=0;
    dp[0]=0;
    for(int i=0;i<n;i++)
    {
        ...
        {
            if(dp[j]<0) continue;
            for(int c=0;c<min(k,sh[i].first+1);c++)
            {
                int cursum=sh[i].first-c;
                if(cursum%k!=0&&cursum%k+sh[i].second<k) continue;
                ll d=cursum/k+(cursum%k+sh[i].second)/k;
                ll bluerem=cursum+sh[i].second-d*k;
                bluerem+=(tot-j-1LL*dp[j]*k);
                d+=bluerem/k;
                d+=(c+j)/k;
                int nxt=(c+j)%k;
                //dbg(i,j,c,d);
                g[nxt]=max(g[nxt],dp[j]+d);
            }
        }
        dp=g;
        //dbg(i,dp);
        tot+=sh[i].first+sh[i].second;
    }
    ll mx=0;
