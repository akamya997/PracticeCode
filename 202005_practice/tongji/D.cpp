...
bool dfs(int T,vc<pi> pos)
{
    ...
    map<pi,int> ext;
    for(auto &u:pos) ext[u]=1;
    for(auto &u:pos)
    {
        vc<pi> nxt;
        for(auto v:pos) if(v!=u) nxt.push_back(v);
        for(int j=0;j<4;j++)
        {
            ...
            while(true)
            {
                ...
                if(zx<0||zy<0||zx>=h||zy>=w||maze[zx][zy]=='X'||ext.count(make_pair(zx,zy))) break;
                nx=zx,ny=zy;
            }
            nxt.emplace_back(nx,ny);
            if(dfs(T+1,nxt)) return true;
            nxt.pop_back();
        }
    }
    return false;
...
int main()
{
    ...
    for(int i=0;i<h;i++)
    {
        ...
        for(int j=0;j<w;j++)
        {
            if(maze[i][j]=='D') tar.emplace_back(i,j);
            else if(maze[i][j]=='R') pos.emplace_back(i,j);
        }
    }
    if(dfs(0,pos)) cout<<"YES\n";
    else cout<<"NO\n";
}