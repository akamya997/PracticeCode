...
int main()
{
    ...
    dp[tar][tar][0]=dp[tar][tar][1]=0;
    for(int len=2;len<=n;len++)
    {
        for(int l=0,r=len-1;r<n;l++,r++)
        {
            int tl=min({dp[l+1][r][0]+p[l+1]-p[l],dp[l+1][r][1]+p[r]-p[l]});
            if(tl<=t[l])
                dp[l][r][0]=min(dp[l][r][0],tl);
            int tr=min({dp[l][r-1][1]+p[r]-p[r-1],dp[l][r-1][0]+p[r]-p[l]});
            if(tr<=t[r])
                dp[l][r][1]=min(dp[l][r][1],tr);
