...
struct Treap
{
    Treap *l, *r;
    int val, prior, sz;
    Treap(int _val) : val(_val), l(NULL), r(NULL), prior(rnd()), sz(1) {}
};
typedef Treap *pt;
inline int SZ(pt p)
{
...
void split(pt o, int k, pt &l, pt &r)
{
    ...
    if(l) l->sz=SZ(l->l)+SZ(l->r)+1;
    if(r) r->sz=SZ(r->l)+SZ(r->r)+1;
}
...
int main()
{
    ...
    while(ti)
    {
        ...
        vi nxt(n);
        for(int i=0;i<n;i++) nxt[i]=p[p[i]-1];
        p=nxt;
        ti>>=1;
    }
    root=NULL;
    for(int i=1;i<=n;i++) merge(root,root,new Treap(ans[i-1]));
    p.clear();
