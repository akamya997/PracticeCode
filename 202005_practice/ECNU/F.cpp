...
void dfs(int u,int fa=-1)
{
    ...
    mxlen=max(mxlen,dp[u]);
    sort(len.begin(),len.end(),greater<int>());
    //dbg(u,len);
    if(len.size()>=2&&val[u]==1)
        mxlen=max(mxlen,len[0]+len[1]+1);
}
pi ans;
bool gt(pi a,pi b)
{
...
void dfs2(int u,int fa=-1)
{
    ...
    if(val[u]==2&&tot.size()>=2)
    {
        int l1=tot[0],l2=tot[1];
        if(l2==mxlen) ans=make_pair(2,l1+l2+1);
    }
    for(auto v:G[u])if(v!=fa)
    {
        len.erase(len.find(dp[v]));
        int svu=dp[u],svv=dp[v];
        if(val[u]==1)
        {
            if(len.size())
                dp[u]=*len.rbegin()+1;
            else dp[u]=1;
            if(val[v]==1) dp[v]=max(dp[v],dp[u]+1);
        }
        else dp[u]=0;
        dfs2(v,u);
        dp[v]=svv;
