...
int main()
{
    ...
    for(int i=1;i<=n;i++)
    {
        if(cur[i]!=G[i].end())
        {
            ...
            //dbg(ans);
            deque<int> q;
            for(auto u:ans) q.push_back(u);
            while(q.size()>1)
            {
                int l=q.front(),r=q.back();
                if(deg[edge[l].first]%2==0||deg[edge[l].second]%2==0)
                {
                    deg[edge[l].first]--;
                    deg[edge[l].second]--;
                    op.push_back(l);
                ...
                else if(deg[edge[r].first]%2==0||deg[edge[r].second]%2==0)
                {
                    deg[edge[r].first]--;
                    deg[edge[r].second]--;
                    op.push_back(r);
