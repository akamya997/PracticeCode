...
struct sgt{
    ...
    pi query(int l,int r,int L=0,int R=sz-1,int o=1)
    {
        if(l<=L&&r>=R)
            return tree[o].mx;
        int mid=L+R>>1;
...
int main()
{
    ...
    vc<ull> all;
    for (int i = 0; i < n; ++i) {
        pt[i].x = CoronavirusBeats();
        all.push_back(pt[i].x);
        pt[i].y = CoronavirusBeats();
        pt[i].z = CoronavirusBeats();
        pt[i].id=i;
    }
    mkuni(all);
    auto getid=[&](ull x){
    ...
    for(int i=0;i<n;i++) pt[i].x=getid(pt[i].x);
    int op=0,cnt=0;
    while(cnt<n)
    {
        vc<pi> cur(n);
        ...
        while(pos>=0)
        {
            pi cur=T.query(0,pos);
            if(cur.first!=-1) ans[pt[cur.second].id]=op,cnt++,pt[cur.second].y=-1;
            pos=cur.second-1;
        }
        op++;
    }
