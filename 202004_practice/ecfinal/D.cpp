...
void dfs(int u,int fa=-1)
{
    ...
    dp[u]=min(a[u],a[u]+k-(sz[u]-1)*2);
    ll tmp=a[u]+k-(sz[u]-1)*2-2;
    sort(son.begin(),son.end(),[&](int x,int y){return min(dp[x],dp[y]-1-sz[x]*2)>min(dp[y],dp[x]-1-sz[y]*2);});
    int cur=0;
    vc<ll> suf(son.size()+1,1e18);
    vc<ll> t(son.size());
    ...
    for(int i=0;i<son.size();i++)
    {
        ll now=min({pre,suf[i+1]+sz[son[i]]*2,g[son[i]]-cur+2*sz[son[i]]-1,(ll)a[u]});
        dbg(u,son[i],now,t[i]);
        g[u]=max(g[u],now);
        pre=min(pre,t[i]);
    }
