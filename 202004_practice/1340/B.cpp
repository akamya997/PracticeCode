...
int main()
{
    ...
    for(int i=0;i<(1<<7);i++)
    {
        for(int j=0;j<10;j++)
        {
            ...
            for(int k=0;k<7;k++)
                cc+=(1<<k)*(a[k]=='1');
            if((cc&i)==i)
                cost[i][j]=__builtin_popcount(cc^i);
            else cost[i][j]=-1;
            //dbg(i,j,cost[i][j]);
        }
    }
    ...
    for(int i=n;i>=1;i--)
    {
        ...
        for(int k=0;k<=9;k++)
        {
            if(cost[cur][k]==-1) continue;
            for(int j=cost[cur][k];j<=kk;j++)
            {
                dp[i][j]|=dp[i+1][j-cost[cur][k]];
                //dbg(i,j,k,dp[i][j]);
            }
        }
    }
    if(!dp[1][kk]) print(-1);
        ...
        for(int i=1;i<=n;i++)
        {
            ...
            for(int j=9;j>=0;j--)
            {
                if(cost[cur][j]==-1) continue;
                if(dp[i+1][cnt-cost[cur][j]])
                {
                    cnt-=cost[cur][j];
                    ans+='0'+j;
                    break;
