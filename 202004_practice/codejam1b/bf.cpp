...
map<vi,int> ext;
map<vi,pair<vi,pi>> pre;
vi a;
void pans(vi cur)
{
    if(cur==a) return;
    pans(pre[cur].first);
    cout<<"cout<<\""<<pre[cur].second.first<<" "<<pre[cur].second.second<<"\\n\";"<<endl;
...
int solve(int r,int c)
{
    ...
    //int r,c;
    //cin>>r>>c;
    cout<<"if(r=="<<r<<"&&"<<"c=="<<c<<")"<<endl;
    for(int i=1;i<=c;i++)
    {
        for(int j=1;j<=r;j++) a.emplace_back(j);
    }
    queue<vi> q;
    q.push(a);
    auto check=[&](vi &a)
    ...
    while(!q.empty())
    {
        ...
        for(int i=1;i<r*c;i++)
        {
            l.push_back(u[i-1]);
            vi rv;
            for(int j=1;i+j<=r*c;j++)
            {
                rv.push_back(u[i+j-1]);
                vi nxt=rv;
                int sz=l.size()+rv.size();
                for(auto uu:l) nxt.push_back(uu);
                for(int i=sz;i<r*c;i++) nxt.push_back(u[i]);
                if(ext.count(nxt)) continue;
                pre[nxt]=make_pair(u,make_pair(i,j));
                ext[nxt]=ext[u]+1;
                q.push(nxt);
            }
        }
