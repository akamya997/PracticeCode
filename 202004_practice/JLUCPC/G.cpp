...
int main()
{
    ...
    {
        int cc=0;
        for(int j=0;j<n;j++) cc+=a[j]>>i&1;
        cnt[i]=cc;
        mi[i]+=min(cc*(1LL<<i),(n-cc)*(1LL<<i));
        if(i) mi[i]+=mi[i-1];
    }
    //dbg(cnt);
    for(int i=31;i<51;i++) mi[i]=mi[i-1];
    int q;
    cin>>q;
    while(q--)
    {
        ...
        for(int i=50;i>=0;i--)
        {
            __int128 d=(__int128)(1LL<<i)*(n-cnt[i]);
            ll rem=i?mi[i-1]:0;
            if(rem+d+cur<=m)
            {
                ans+=1LL<<i;
            ...
            else cur+=(1LL<<i)*(cnt[i]);
            //dbg(i,d,cur,rem,mi[i],ans);
        }
        //ll test=0;
        //for(int i=0;i<n;i++) test+=(a[i]^ans);
        //print(test);
        print(ans);
    }
}