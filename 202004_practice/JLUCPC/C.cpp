...
{
    int cur=0;
    vi pre(s.length()+2),suf(s.length()+2);
    for(int i=1;i<=s.length();i++)
    {
        int cur=s[i-1]-'0';
        pre[i]=(pre[i-1]*10LL+cur)%mod;
    }
    for(int i=s.length();i>=1;i--)
    {
        int cur=s[i-1]-'0';
        suf[i]=(suf[i+1]+1LL*ten[s.length()-i]*cur)%mod;
    }
    int ans=0;
    for(int i=1;i<=s.length();i++)
    {
        ...
        if(cur==7)
        {
            ans=(ans+suf[i+1]+1)%mod;
            ans=(ans+1LL*pre[i-1]*ten[s.length()-i]%mod)%mod;
        }
        else if(cur<7){
            if(i>1)
                ans=(ans+1LL*pre[i-1]*ten[s.length()-i]%mod)%mod;
            ...
            ans=(ans+1LL*(pre[i-1]+1)%mod*ten[s.length()-i]%mod)%mod;
        }
        //dbg(i,pre[i],suf[i],ans);
    }
    return ans;
}
int main()
{
    ...
    int T;
    ten[0]=1;
    for(int i=1;i<maxn;i++) ten[i]=1LL*ten[i-1]*10%mod;
    cin>>T;
