...
void dfs2(int i,int j)
{
    ...
    {
        int nx=i+dx[k],ny=j+dy[k];
        if(nx>=0&&nx<n&&ny>=0&&ny<n&&m[nx][ny]=='.'&&ins[nx][ny])
            dfs2(nx,ny);
    }
}
int main()
{
                ...
                for(int ii=0;ii<n;ii++)
                {
                    for(int jj=0;jj<n;jj++)
                    {
                        if(m[ii][jj]=='.')
                        {
                            for(auto u:cur)
                            {
                                if(adj(make_pair(ii,jj),u))
                                {
                                    ins[ii][jj]=1;
                                    curp.emplace_back(ii,jj);
                                    break;
                                }
                            }
                        }
    ...
    if(!ans) cout<<"Second player will win\n";
