...
int main()
{
    ...
    auto del=[&](int x)
    {
        int delta=tar[x]-cur[x];
        int k=pos[x].size();
        int per=(delta+k-1)/k;
        for(auto &u:pos[x])
        {
            cur[x]+=val[u.second]-u.first;
            st[u.second].erase(make_pair(per+u.first,x));
        }
    };
    auto add=[&](int x)
    {
        ...
        for(auto &u:pos[x])
        {
            st[u.second].insert(make_pair(per+val[u.second],x));
            u.first=val[u.second];
        }
        return false;
    };
    while(m--)
    {
        ...
        if(tp==1)
        {
            ...
            val[x]+=y;
            vi ans;
            while(st[x].size()&&(*st[x].begin()).first<=val[x])
            {
                int dd=(*st[x].begin()).second;
                del(dd);
                if(add(dd))
                {
                    lastans++;
        ...
        //for(int i=1;i<=id;i++) dbg(i,cur[i]);
