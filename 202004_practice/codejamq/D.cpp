...
int main()
{
    ...
    while(T--)
    {
        ...
        if(b==10)
        {
            ...
            vc<pair<int,char> > same,dif;
            auto modify=[&](){
                if(same.size())
                {
                    char cc=ask(same.at(0).first);
                    if(cc!=same.at(0).second)
                    {
                        for(auto &u:same)
                        {
                            if(u.second=='1') u.second='0';
                            else u.second='1';
                        }
                    }
                }
                ...
                if(dif.size())
                {
                    char cc=ask(dif.at(0).first);
                    if(cc!=dif.at(0).second)
                    {
                        for(auto& u:dif)
                        {
            ...
            for(int i=1;i<=b/2;i++)
            {
                char cl=ask(i),cr=ask(b+1-i);
                if(cl==cr)
                    same.emplace_back(i,cl);
                else dif.emplace_back(i,cl);
                if(ti%10==1)
                    modify();
            }
            string ans;
            ans.resize(b);
            for(auto u:same)
                ans[u.first-1]=ans[b-u.first]=u.second;
            for(auto u:dif)
            {
                ans[u.first-1]=u.second;
                if(u.second=='1')
                    ans[b-u.first]='0';
                else ans[b-u.first]='1';
            }
            cout<<ans<<endl;
        }
        string res;
