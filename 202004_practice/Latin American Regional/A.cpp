...
map<vc<string>,int> id;
int tot=0;
inline int getid(vc<string> &a)
{
...
int main()
{
    ...
    for(int i=0;i<n;i++)
    {
        ...
        for(int j=1;j<(1<<A);j++)
        {
            vc<string> cc;
            for(int k=0;k<A;k++)
                if(j>>k&1) cc.push_back(cur[k]);
            getid(cc);
        }
    }
    int S=tot+tot+1,T=S+1;
    for(int i=0;i<tot;i++)
    {
        F.AddEdge(S,i,1);
        F.AddEdge(i+tot,T,1);
    ...
    for(auto& [st,cid]:id)
    {
        ...
        for(int j=0;j<st.size();j++)
        {
            ...
            int ccid=getid(tmp);
            F.AddEdge(cid,ccid+tot,1);
        }
    }
    cout<<tot-F.Maxflow(S,T)<<'\n';
}