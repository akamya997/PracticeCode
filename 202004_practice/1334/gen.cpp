...
int main()
{
    ...
    cout<<n<<endl;
    vi a;
    for(int i=0;i<n;i++) a.push_back(rand_int(1,n));
    print(a);
    for(int i=0;i<n;i++) cout<<-rand_int(0,10)<<" ";
    cout<<endl;
    int m=3;
    cout<<m<<endl;
    int suc=1;
    ...
    {
        b.clear();
        for(int i=0;i<m;i++) b.push_back(a[rand_int(0,n-1)]);
        sort(b.begin(),b.end());
        bool ok=1;
        for(int i=1;i<m;i++) if(b[i]==b[i-1]) ok=0;
        if(ok) suc=0;
    }
    print(b);
}