...
struct sgt{
    ...
    void modify(int p,ll val,int L=0,int R=sz-1,int o=1)
    {
        if(L==R)
        {
            tree[o]=D(val);
    ...
    void add(int l,int r,ll val,int L=0,int R=sz-1,int o=1)
    {
        if(l<=L&&r>=R)
        {
            tree[o].add(val);
...
int main()
{
    ...
    for(int i=1;i<=n;i++)
    {
        if(cc<m&&b[cc]==i)
        {
            ...
            vc<ll> opt;
            for(auto u:pos[i])
                opt.push_back(T.query(0,u)-suf.query(u));
            //dbg(opt);
            for(auto u:pre)
                T.modify(u,1e18);
            ...
            {
                if(opt[j]>1e16) continue;
                T.modify(pos[i][j],opt[j]+suf.query(pos[i][j]+1));
                suc=1;
            }
            if(!suc)
            {
