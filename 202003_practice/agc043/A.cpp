...
int main()
{
    ...
    while(!q.empty())
    {
        ...
        int nx=u.first+1,ny=u.second+1;
        if(nx<h)
        {
            int d=0;
            if(maze[u.first][u.second]=='.'&&maze[nx][u.second]=='#') d++;
            if(vis[nx][u.second]>vis[u.first][u.second]+d)
            {
                vis[nx][u.second]=vis[u.first][u.second]+d;
                if(d==0)
                    q.push_front(make_pair(nx,u.second));
                else q.push_back(make_pair(nx,u.second));
            }
        }
        if(ny<w)
        {
            int d=0;
            if(maze[u.first][u.second]=='.'&&maze[u.first][ny]=='#') d++;
            if(vis[u.first][ny]>vis[u.first][u.second]+d)
            {
                vis[u.first][ny]=vis[u.first][u.second]+d;
                if(d==0)
                    q.push_front(make_pair(u.first,ny));
                else q.push_back(make_pair(u.first,ny));
            }
        }
    }
    cout<<vis[h-1][w-1]<<'\n';
